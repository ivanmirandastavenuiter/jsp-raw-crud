...
CREATE OR REPLACE TRIGGER validateUserBeforeInsert BEFORE INSERT ON user
    ...
    BEGIN
        ...
        DECLARE usNamRef VARCHAR(51);
        DECLARE usSurRef VARCHAR(56);
        DECLARE usUseRef VARCHAR(51);
        DECLARE usEmaRef VARCHAR(71);
        DECLARE usPhoRef VARCHAR(10);
        DECLARE usPasRef VARCHAR(255);
        ...
        DECLARE existingUsername VARCHAR(51);
        ...
        IF usNamRef IS NULL OR
            ...
            CHAR_LENGTH(TRIM(usNamRef)) > 50 THEN
                IF CHAR_LENGTH(TRIM(usNamRef)) > 50 THEN
                    SIGNAL dataTooLong;
                ELSE
                    SIGNAL nameError;
                END IF;
        ELSEIF usSurRef IS NULL OR 
            ...
            CHAR_LENGTH(TRIM(usSurRef)) > 55 THEN
                IF CHAR_LENGTH(TRIM(usSurRef)) > 55 THEN
                    SIGNAL dataTooLong;
                ELSE
                    SIGNAL surnameError;
                END IF;
        ELSEIF usUseRef IS NULL OR 
            ...
            CHAR_LENGTH(TRIM(usUseRef)) > 50 THEN
                IF CHAR_LENGTH(TRIM(usUseRef)) > 50 THEN
                    SIGNAL dataTooLong;
                ELSE
        ...
        ELSEIF usPasRef IS NULL OR
            ...
            CHAR_LENGTH(TRIM(usPasRef)) > 255 THEN 
                IF CHAR_LENGTH(TRIM(usPasRef)) > 255 THEN
                    SIGNAL dataTooLong;
                ELSE
                    SIGNAL passwordError;
                END IF;
        END IF;
...
-- Trigger for cipher before insert
delimiter |
CREATE OR REPLACE TRIGGER validateCipherBeforeInsert BEFORE INSERT ON cipher
    FOR EACH ROW
    BEGIN

        -- Declaration of variables
        DECLARE ciKeyRef VARCHAR(255);
        DECLARE ciVecRef VARCHAR(255);
        DECLARE usIdRef SMALLINT;
		
		-- Setting conditions and handlers
        DECLARE keyError CONDITION FOR SQLSTATE '45000';
		DECLARE vectorError CONDITION FOR SQLSTATE '45001';
		DECLARE userIdError CONDITION FOR SQLSTATE '45002';

        -- Setting handlers
        DECLARE EXIT HANDLER FOR keyError
        BEGIN
            RESIGNAL SET MESSAGE_TEXT = 'Password must have a key for the cipher.', MYSQL_ERRNO = 1001;
        END;
		DECLARE EXIT HANDLER FOR vectorError
        BEGIN
            RESIGNAL SET MESSAGE_TEXT = 'Cipher must have a vector for initialization', MYSQL_ERRNO = 1001;
        END;
		DECLARE EXIT HANDLER FOR userIdError
        BEGIN
            RESIGNAL SET MESSAGE_TEXT = 'Keys must be related to a certain user', MYSQL_ERRNO = 1001;
        END;

        -- Variables assignment
        SET ciKeyRef = NEW.ciKey;
        SET ciVecRef = NEW.ciVec;
        SET usIdRef = NEW.usId;

        -- Validations to be run against fields
        IF ciKeyRef IS NULL OR
            CHAR_LENGTH(TRIM(ciKeyRef)) = 0 OR
            CHAR_LENGTH(TRIM(ciKeyRef)) > 255 THEN
                SIGNAL keyError;
        ELSEIF ciVecRef IS NULL OR 
            CHAR_LENGTH(TRIM(ciVecRef)) = 0 OR
            CHAR_LENGTH(TRIM(ciVecRef)) > 255 THEN
                SIGNAL vectorError;
        ELSEIF (SELECT COUNT(usId) FROM user WHERE usId = usIdRef) = 0 THEN 
            SIGNAL userIdError;
        END IF;
...
-- Trigger for cipher before insert
delimiter |
CREATE OR REPLACE TRIGGER validateCipherBeforeUpdate BEFORE UPDATE ON cipher
    FOR EACH ROW
    BEGIN
